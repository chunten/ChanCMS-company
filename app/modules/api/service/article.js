...
class ArticleService {
    ...
    try {
      ...
      await knex.transaction(async (trx) => {
        ...
        if (result[0]) {
          ...
          // 新增文章和标签关联
          const tags = defaultParams.tagId.split(",").map(Number);
          const tagsql = [];
          tags.forEach((item) => {
            tagsql.push(`(${id},${item})`);
          });
  ...
  async detail(id) {
    try {
      ...
      data[0].pics=!data[0].pics?[]:data[0].pics
      data[0].videos=!data[0].videos?[]:data[0].videos
  ...
  async search(key = "", cur = 1, pageSize = 10, cid = 0) {
    try {
      ...
      const offset = parseInt((cur - 1) * pageSize);
      let sql_list = "";
      const listStart = `SELECT a.id,a.title,a.attr,a.img,a.tagId,a.description,a.cid,a.pv,a.createdAt,a.status,c.name,c.path,c.type FROM ${this.model} a LEFT JOIN cms_category c ON a.cid=c.id WHERE a.title LIKE  \'%${key}%\' `;
