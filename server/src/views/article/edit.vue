<template>
      ...
      >
        <div v-show="activeIndex == 0">
          ...
          <el-form-item label="图片">
            <div>
              <div class="flex mb-9" v-for="(item,index) in params.pics" :key="index">
                <el-upload
                  class="avatar-uploader"
                  :http-request="uploadx"
                  :show-file-list="false"
                  :data={index:index}
                  :before-upload="beforeUploadx"
                >
                  <el-popover
                    placement="top-start"
                    title="上传"
                ...
                >
                  ...
                  <el-image style="width: 100%" :src="item.path" />
                </el-popover>
                <el-input class="ml-10 w-300" v-model="item.name" placeholder="图片名称"></el-input>
                <el-input class="ml-10 w-300" v-model="item.path" placeholder="图片地址"></el-input>
                <el-button @click="delx(index)" class="ml-5">删除</el-button>
              </div>
  ...
  <div style="width: 100%;margin:0 auto;display: flex;justify-content: center;position: fixed;bottom:0;background: #fff;border-top:1px solid #ddd;padding:10px 0px;">
    <el-button type="primary" @click="submit('params')" size="large">保存</el-button>
  </div>
</template>
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (res.code === 200) {
          ...
          this.field.forEach((item, index) => {
            if (this.fieldParams[item.ename].includes("[{")) {
              item.values = JSON.parse(
                this.fieldParams[item.ename],
              );
            } else {
    ...
    async addPic(){
      console.log("xxxx")
      this.params.pics.push({name:'',path:''})
    },
    // 文章详情
    async detail() {
      try {
        ...
        if (res.code === 200) {
          ...
          console.log(params.pics)
          if(params.hasOwnProperty('pics') && params.pics.length!==0){
            params.pics=JSON.parse(params.pics)
          }
          this.params = params;
    ...
    async uploadx(files) {
      ...
      if (res.code === 200) {
        this.params.pics[files.data.index]={name:'',path:res.data.path}
      }
    },
    ...
    async update() {
      try {
        ...
        params.subCid = params.subCid.toString();
