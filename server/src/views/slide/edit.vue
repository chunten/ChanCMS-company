<template>
  <div class="mr-10 ml-10 mb-20 pd-20 content-wrap">
    <el-form ref="params" :model="params" label-width="140px" class="mt-20">
      <el-form-item
        label="轮播标题"
        :rules="[
          {
  ...
  methods: {
    ...
    async detail() {
      try {
        ...
        if (res.code === 200) {
          const { id, title, imgUrl, linkUrl,content } = res.data;
          this.params = { id, title, imgUrl, linkUrl,content };
        } else {
          this.$message({
            message: res.msg,
            type: "success",
